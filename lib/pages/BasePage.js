...
class BasePage {
   ...
   */
  async hideKeyboard() {
    const platform = this.platform || PlatformUtils.detectPlatform();
    ...
    if (platform === 'android') {
      try {
        ...
        log.warn(`Failed to hide keyboard via back button: ${error.message}`);
      }
    }
    log.warn('Could not dismiss keyboard; continuing test execution');
  }
